...
public class BaseDeDatosView extends JInternalFrame {
private final static Logger log = Logger.getLogger(BaseDeDatosView.class);
	...
	/**
	 * Constructor de la clase llamada a la inicializaci�n de los componentes
	 * gr�ficos
	 */
	public BaseDeDatosView(){
		setResizable(false);// Constructor
		//configuraci�n para el archivo log
	...
	/**
	 * inicializa los componentes g�ficos
	 */
	private void initGUI ( ) {// inicio de la ventana
		...
		try {
			/*
			this.addInternalFrameListener( new WindowAdapter( ) {// cerrando la ventana
	...
	/**
	 * Inicia conexi�n con la base de datos 
	 * @param e evento clic 
	 */
	public void iniciarConexion () {
		// Verficaci�n de los par�metros para realizar la conexi�n a la base de
		// datos
		if ( usuario.getText( ).isEmpty( ) == false && contra.getPassword( ).toString( ).isEmpty( ) == false && direccion.getText( ).isEmpty( ) == false ) {
			...
			if(bd.testConexion(direccion.getText(), usuario.getText(),passwd)){
			
				JOptionPane.showMessageDialog(this, "Se ha realizado la conexión exitosamente");
			}
			else
			{
				JOptionPane.showMessageDialog(this, "Algunos de los valores son incorrectos, No se puede realizar la conexión","Atención", JOptionPane.WARNING_MESSAGE);
			}
		} else {
